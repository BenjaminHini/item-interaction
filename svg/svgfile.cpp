...
void Svgfile::addTriangle(double x1, double y1, double x2, double y2, std::string colorFill,
                          double thickness, std::string colorStroke)
{
   double cx1,cy1,cx4,cy4,l=5;

   if(y1!=y2 && x1!=x2)
{
     cx1=x1-( (l)/(2*sqrt( ( 1/pow( (y2-y1)/(x2-x1),2 ) ) +1) ) );
    cy1= ( l/ (2*( (y2-y1)/(x2-x1) )*sqrt( ( 1/pow( (y2-y1)/(x2-x1),2 ) ) +1) ))+y1;
    cx4=x1-(cx1-x1);
    cy4=y1+(y1-cy1);
}
...
void Svgfile::addRectangle(double x, double y, double tx, double ty,Coords pda, std::string color, double inclinaison)
{
    std::ostringstream oss;
    oss<<"rotate("<<inclinaison<<","<<pda.getX()<<","<<pda.getY()<<")";
    m_ostrm<<"<rect "
           <<attrib("x", x)
           <<attrib("y", y)
           <<attrib("width", tx)
